import React,{Component} from 'react';
import './Finish.css';


class Finish extends Component{
  render(){

    return (
      <div className="Finish">
        <form className="paymentForm">
          <div className="total">
            <h5>Total:</h5>
            <h2>R$ {localStorage.length>0?Object.keys(localStorage).map(key => {
                    let product = JSON.parse(localStorage.getItem(key));
                    return parseFloat(product.price) * product.amount;
                  }).reduce((acc,cur) => acc+cur):0}
            </h2>
          </div>
          <h2>
            <svg className="cartIcons" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
              <path d="M256,0C153.755,0,70.573,83.182,70.573,185.426c0,126.888,165.939,313.167,173.004,321.035 c6.636,7.391,18.222,7.378,24.846,0c7.065-7.868,173.004-194.147,173.004-321.035C441.425,83.182,358.244,0,256,0z M256,278.719 c-51.442,0-93.292-41.851-93.292-93.293S204.559,92.134,256,92.134s93.291,41.851,93.291,93.293S307.441,278.719,256,278.719z"/>
            </svg> 
            Informações de entrega
          </h2>
          <div className="accessData">
            <div>
              Nome e sobrenome:<br/>
              <input placeholder="Nome e sobrenome" type="text" name="on_behalf"/>
            </div>

            <div>
              Telefone:<br/>
              <input placeholder="(00) 9999-9999" type="text" name="phone"/>
            </div>

            <div>
              CPF:<br/>
              <input placeholder="000.000.000-00" type="text" name="document"/>
            </div>

            <div>
              E-mail:<br/>
              <input placeholder="email@example.com" type="text" name="username"/>
            </div>

            <div>
              Senha para acesso:<br/>
              <input placeholder="*****" type="text" name="password"/>
            </div>

            <div>
              CEP:<br/>
              <input placeholder="00000-000" type="text" name="postal_code"/>
            </div>

            <div>
              Cidade:<br/>
              <input placeholder="Cidade" type="text" name="city"/>
            </div>

            <div>
              Estado:<br/>
              <input placeholder="Estado" type="text" id="id_state"/>
            </div>

            <div>
              Endereço:<br/>
              <input placeholder="Endereço" type="text" name="address"/>
            </div>

            <div>
              Bairro:<br/>
              <input placeholder="Bairro" type="text" name="neighborhood"/>
            </div>

            <div>
              Número / complemento:<br/>
              <input placeholder="Núm / Comp" type="text" name="address_number"/>
            </div>

            <div>
              Ponto de referência (opcional)<br/>
              <input placeholder="Referência" type="text" name="reference"/>
            </div>
          </div>

          <h2>
            <svg className="cartIcons" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 611 611">
              <path d="M588.63,113.193L213.812,33.871c-15.858-3.355-31.576,6.876-34.931,22.734l-7.121,45.762l432.477,91.519l7.121-45.762 C614.713,132.272,604.488,116.549,588.63,113.193z"/> <path d="M431.009,203.591c-4.378-15.766-20.854-25.085-36.615-20.714L323.24,202.63l-167.742-35.5l-18.448,87.165L21.786,286.287 c-15.76,4.372-25.079,20.848-20.708,36.609l64.958,234.078c4.378,15.76,20.855,25.085,36.615,20.708l372.608-103.403 c15.76-4.378,25.079-20.848,20.708-36.615l-11.15-40.184l41.789,8.835c15.858,3.361,31.576-6.87,34.931-22.728l26.439-124.937 L437.45,226.797L431.009,203.591z M474.04,322.559l9.215-43.552c1.384-6.521,7.85-10.727,14.37-9.35l43.552,9.221 c6.527,1.384,10.733,7.843,9.356,14.37l-9.215,43.552c-1.384,6.521-7.849,10.733-14.37,9.35l-43.552-9.215 C476.863,335.546,472.656,329.08,474.04,322.559z M28.27,309.646l103.115-28.606l243.299-67.517l26.181-7.274 c0.478-0.129,0.955-0.19,1.421-0.19c2.1,0,4.611,1.378,5.345,4.017l3.074,11.07l9.631,34.704L37.148,362.186l-12.705-45.768 C23.647,313.546,25.399,310.442,28.27,309.646z M472.601,444.141c0.49,1.776-0.024,3.245-0.545,4.164 c-0.514,0.918-1.506,2.119-3.282,2.608L96.173,554.316c-0.471,0.129-0.955,0.196-1.421,0.196c-2.1,0-4.611-1.384-5.345-4.023 L51.519,413.955l383.188-106.342l23.371,84.208L472.601,444.141z"/> <path d="M156.379,453.484c-1.788-6.429-8.499-10.225-14.928-8.443l-43.515,12.08c-6.423,1.782-10.225,8.499-8.437,14.928 l12.074,43.509c1.788,6.429,8.499,10.225,14.928,8.437l43.515-12.074c6.429-1.782,10.225-8.499,8.443-14.928L156.379,453.484z"/>          
            </svg>
            Informações de pagamento
          </h2>
          <div className="paymentData">
          <div>
              Nome no cartão<br/>
              <input placeholder="Nome no cartão" type="text" name="owner"/>
            </div>

            <div>
              Número do cartão<br/>
              <input placeholder="0000 0000 0000 0000" type="text" name="card_number"/>
            </div>

            <div>
              Data de expiração<br/>
              <input placeholder="00/0000" type="text" name="expiration_date"/>
            </div>

            <div>
              CVV<br/>
              <input placeholder="000 " type="text" name="card_cvv"/>
            </div>
          </div>

          <div className="finishFooter">
            <button type="button" onClick={() => this.props.history.push('/cart')} className="back">{"<"} Voltar</button>
            <button type="button" onClick={() => this.props.history.push('/success')} className="finish">Finalizar compra{">"}</button>
          </div>
        </form>
      </div>
    );
  }
}

export default Finish;
